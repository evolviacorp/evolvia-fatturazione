const STEPS = [
  ...
  { num: 2, label: 'Agenti' },
  { num: 3, label: 'Trattenuto' },
  { num: 4, label: 'Quote soci' },
]

export default function StepIndicator({ currentStep, skipStep3 }) {
  function getState(num) {
    if (num === 3 && skipStep3 && currentStep === 4) return 'skipped'
          ...
          <div key={step.num} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold border-2 transition-all ${
                ...
                state === 'skipped' ? 'border-dashed border-slate-300 text-slate-300' :
                                      'border-slate-200 text-slate-300 bg-white'
              }`}>
                {state === 'done' ? '✓' : state === 'skipped' ? '–' : step.num}
              </div>
